...
func (m *Matcher) Match(vp vulnerability.Provider, p pkg.Package) ([]match.Match, []match.IgnoreFilter, error) {
	...
	provider := result.NewProvider(vp, func(criteria []vulnerability.Criteria, v vulnerability.Vulnerability) match.Details {
		...
		if ok {
			...
			return match.Details{internal.CPEMatchDetails(match.ApkMatcher, v, searchedByCPE, p, searchVersion)}
		}
		return internal.DistroMatchDetails(matchType, match.ApkMatcher, p, v)
	})
...
func (m *Matcher) findMatchesForPackage(provider result.Provider, p pkg.Package) ([]match.Match, []match.IgnoreFilter, error) {
	...
	return secDbMatches.Merge(cpeVulns).ToMatches(p), ignored, nil
}
